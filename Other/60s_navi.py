...
# 60s 迭代步长数据
HPOP_1 = ( pd.read_csv("STK/60_30s/Sat_1_60s.csv", nrows=NUMBER, usecols=range(1,7)) ).values
HPOP_2 = ( pd.read_csv("STK/60_30s/Sat_2_60s.csv", nrows=NUMBER, usecols=range(1,7)) ).values
...
#############################################################################################################
	...
	def plot_filter_higher(self, X, number):
		...
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/Freq_60s/A星位置收敛曲线.png", bbox_inches='tight')
		...
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/Freq_60s/A星速度收敛曲线.png", bbox_inches='tight')
		...
		plt.plot(time_range, up_error, "k:")
		plt.plot(time_range, low_error, "k:", label=" $\mathrm{\pm 30 m}$")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/Freq_60s/B星位置收敛曲线.png", bbox_inches='tight')
		...
		plt.plot(time_range, v_upError, "k:")
		plt.plot(time_range, v_lowError, "k:", label=" $\mathrm{\pm 0.02 m/s}$")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/Freq_60s/B星速度收敛曲线.png", bbox_inches='tight')
		...
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/Freq_60s/A星绝对误差曲线.png", bbox_inches='tight')
		
		plt.figure(6)
		plt.xlabel("时间 / $\mathrm{h}$"); plt.ylabel("位置误差 / ($\mathrm{m}$)")
		plt.plot(time_range, delta_norm_r2 * 1000, "b--", label="B星位置误差")
		plt.plot(time_range, abs_error, "k:", label="+50 m")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/Freq_60s/B星绝对误差曲线.png", bbox_inches='tight')
...
if __name__ == "__main__":
	...
	# np.save("npy/ia=ib=28, freq=60s.npy", X)
	X = np.load("npy/ia=ib=28, frq=60s.npy")
