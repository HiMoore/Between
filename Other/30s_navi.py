...
HPOP_1 = ( pd.read_csv("STK/60_30s/Sat_1_30s.csv", nrows=NUMBER, usecols=range(1,7)) ).values
HPOP_2 = ( pd.read_csv("STK/60_30s/Sat_2_30s.csv", nrows=NUMBER, usecols=range(1,7)) ).values
...
#############################################################################################################
	...
	def state_equation(self, X, dt=STEP):
		'''UKF的状态方程, X_k+1 = f(x_k) + w_k, ndarray;	 dt=STEP, 为UKF的predict步长;	 计算时间0.20s'''
		global Time
		X1, X2 = X[0:6], X[6:12]
	...
	def plot_filter_higher(self, X, number):
		...
		up_error = 40 + 1e3 * np.power(e, -time_range)
		low_error = -40 - 1e3 * np.power(e, -time_range)
		v_upError = 0.03 + np.power(e, -time_range)
		v_lowError = -0.03 - np.power(e, -time_range)
		plt.figure(1)
		...
		plt.plot(time_range, low_error, "k:", label=" $\mathrm{\pm 40 m}$")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/Freq_30s/A星位置收敛曲线.png", bbox_inches='tight')
		...
		plt.plot(time_range, v_lowError, "k:", label=" $\mathrm{\pm 0.03 m/s}$")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/Freq_30s/A星速度收敛曲线.png", bbox_inches='tight')
		...
		plt.plot(time_range, up_error, "k:")
		plt.plot(time_range, low_error, "k:", label=" $\mathrm{\pm 40 m}$")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/Freq_30s/B星位置收敛曲线.png", bbox_inches='tight')
		...
		plt.plot(time_range, v_upError, "k:")
		plt.plot(time_range, v_lowError, "k:", label=" $\mathrm{\pm 0.03 m/s}$")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/Freq_30s/B星速度收敛曲线.png", bbox_inches='tight')
		...
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/Freq_30s/A星绝对误差曲线.png", bbox_inches='tight')
		
		plt.figure(6)
		plt.xlabel("时间 / $\mathrm{h}$"); plt.ylabel("位置误差 / ($\mathrm{m}$)")
		plt.plot(time_range, delta_norm_r2 * 1000, "b--", label="B星位置误差")
		plt.plot(time_range, abs_error, "k:", label="+50 m")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/Freq_30s/B星绝对误差曲线.png", bbox_inches='tight')
...
if __name__ == "__main__":
	...
	# np.save("npy/ia=ib=28, freq=30s_b.npy", X)
	X = np.load("npy/ia=ib=28, freq=30s.npy")
