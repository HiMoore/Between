...
Time, NUMBER = 0, 720
Qk = np.diag([ 1e-10, 1e-10, 1e-10, 1e-14, 1e-14, 1e-14, 1e-10, 1e-10, 1e-10, 1e-14, 1e-14, 1e-14 ]) * 1e-6	# 12*12
Rk = np.power( np.diag( [1e-3, radians(10/3600), radians(10/3600), radians(10/3600)] ), 2 )	# 4*4, sigma_r*I
# 基础卫星数据，轨道倾角分别为0和28.5°
HPOP_1 = ( pd.read_csv("STK/basic/Sat_1_i0.csv", nrows=NUMBER, usecols=range(1,7)) ).values
HPOP_2 = ( pd.read_csv("STK/basic/Sat_2_i28.csv", nrows=NUMBER, usecols=range(1,7)) ).values
...
#############################################################################################################
	...
	def plot_filter(self, X, number):
		...
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/diffOrbit_bigError/A星位置收敛曲线.png", bbox_inches='tight')
		...
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/diffOrbit_bigError/A星速度收敛曲线.png", bbox_inches='tight')
		...
		plt.plot(time_range, up_error, "k:")
		plt.plot(time_range, low_error, "k:", label=" $\mathrm{\pm 50 m}$")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/diffOrbit_bigError/B星位置收敛曲线.png", bbox_inches='tight')
		...
		plt.plot(time_range, v_upError, "k:")
		plt.plot(time_range, v_lowError, "k:", label=" $\mathrm{\pm 0.1 m/s}$")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/diffOrbit_bigError/B星速度收敛曲线.png", bbox_inches='tight')
		...
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/diffOrbit_bigError/A星绝对误差曲线.png", bbox_inches='tight')
		
		plt.figure(6)
		plt.xlabel("时间 / $\mathrm{h}$"); plt.ylabel("位置误差 / ($\mathrm{m}$)")
		plt.plot(time_range, delta_norm_r2 * 1000, "b--", label="B星位置误差")
		plt.plot(time_range, abs_error, "k:", label="+100 m")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/diffOrbit_bigError/B星绝对误差曲线.png", bbox_inches='tight')
...
if __name__ == "__main__":
	...
	np.save("npy/ia=0, ib=28, P0=3e-2, Qk=1e-16.npy", X)
	# X = np.load("npy/ia=0, ib=28, P0=3e-2, Qk=1e-12.npy")
