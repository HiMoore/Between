...
#############################################################################################################
	...
	def unscented_kf(self, number):
		global Time
		P0 = np.diag([ 3e-2, 3e-2, 3e-2, 3e-6, 3e-6, 3e-6, 3e-2, 3e-2, 3e-2, 3e-6, 3e-6, 3e-6 ])
		# P1 = np.diag([ 3e-2, 3e-2, 3e-2, 3e-6, 3e-6, 3e-6, 3e-2, 3e-2, 3e-2, 3e-6, 3e-6, 3e-6 ])
		error = np.random.multivariate_normal(mean=np.zeros(12), cov=P0)
		X0 = np.hstack( (HPOP_1[0], HPOP_2[0]) ) + error
	...
	def plot_filter(self, X, number):
		...
		up_error = 80 + 1e3 * np.power(e, -time_range)
		low_error = -80 - 1e3 * np.power(e, -time_range)
		v_upError = 0.05 + np.power(e, -time_range)
		v_lowError = -0.05 - np.power(e, -time_range)
		plt.figure(1)
		...
		plt.plot(time_range, low_error, "k:", label=" $\mathrm{\pm 80 m}$")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/coplane_bigError/A星位置收敛曲线.png", bbox_inches='tight')
		...
		plt.plot(time_range, v_lowError, "k:", label=" $\mathrm{\pm 0.05 m/s}$")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/coplane_bigError/A星速度收敛曲线.png", bbox_inches='tight')
		...
		plt.plot(time_range, up_error, "k:")
		plt.plot(time_range, low_error, "k:", label=" $\mathrm{\pm 80 m}$")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/coplane_bigError/B星位置收敛曲线.png", bbox_inches='tight')
		...
		plt.plot(time_range, v_upError, "k:")
		plt.plot(time_range, v_lowError, "k:", label=" $\mathrm{\pm 0.05 m/s}$")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/coplane_bigError/B星速度收敛曲线.png", bbox_inches='tight')
		...
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/coplane_bigError/A星绝对误差曲线.png", bbox_inches='tight')
		
		plt.figure(6)
		plt.xlabel("时间 / $\mathrm{h}$"); plt.ylabel("位置误差 / ($\mathrm{m}$)")
		plt.plot(time_range, delta_norm_r2 * 1000, "b--", label="B星位置误差")
		plt.plot(time_range, abs_error, "k:", label="+100 m")
		plt.legend(); plt.tight_layout(); plt.savefig("Figure/FourChapter/coplane_bigError/B星绝对误差曲线.png", bbox_inches='tight')
...
if __name__ == "__main__":
	...
	np.save("npy/i=28, P0=3e, Qk=1e-10.npy", X)
	# X = np.load("npy/i=28, P0=3e-2, P1=3e, Qk=1e-10.npy")
