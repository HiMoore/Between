...
# 恒定常值变量区
MIU_E, MIU_M, MIU_S = 398600441500000, 4902800305555.40, 132712422595659000000		# 引力系数，单位 m^3 / s^2
RE, RM, RS = 6378136.3, 1738000, 69550000		#天体半径，单位 m
...
class Orbit:
	...
	def moon2Earth(self, tdb_jd):
		'''计算地球相对于月球的位置矢量在地心J2000系下的表示, single-time, m
		输入：TDB的儒略日时间										Timestamp			
		输出：返回地球相对于月球的位置矢量(地球J2000下), 单位 m	np.array'''
		M2E_bc = -kernel[3, 301].compute(tdb_jd) 	# 月球 -> 地月中心
		E_bc2E = kernel[3, 399].compute(tdb_jd)		# 地月中心 -> 地球
		return (M2E_bc + E_bc2E) * 1000	# 月球 -> 地球
		
		
	def moon2Sun(self, tdb_jd):		
		'''计算太阳相对于月球的位置矢量在地心J2000系下的表示, single-time, m
		输入：TDB的儒略日时间										Timestamp
		输出：返回太阳相对于月球的位置矢量(地球J2000下), 单位 m,	np.array'''
		...
		return -(S2S_bc + S_bc2E_bc + E_bc2M) * 1000	# 月球 -> 太阳
	...
	def centreGravity(self, r_sat, miu=MIU_M):
		'''计算中心天体的引力加速度，single-time，输入 m, 输出 m/s^2
		输入：惯性系下卫星位置矢量r_sat，np.array, 单位 m;		miu默认为月球;
		输出：返回中心天体的引力加速度, np.array，单位 m/s^2'''
		r_norm = np.linalg.norm(r_sat, 2)
		g0 = -miu*r_sat / pow(r_norm, 3)		# 中心引力 1*3
		return g0	# m/s^2
	...
	def thirdSun(self, r_sat, tdb_jd):
		'''计算太阳对卫星产生的第三天体摄动加速度, single-time, 输入 m, 输出m/s^2
	...
	def thirdEarth(self, r_sat, tdb_jd):
		'''计算地球对卫星产生的第三天体摄动加速度, single-time, 输入 m, 输出 m/s^2
		输入：UTC时间time_utc，卫星相对中心天体的矢量, m, 均为np.array;
		...
		return a_earth	# m/s^2
			

	def solarPress(self, r_sat, tdb_jd):
		'''计算太阳光压摄动, single-time, 输入 m, 输出 m/s^2
	...
	def integrate_orbit(self, rv_0, num):
		...
		ode_y = solve_ivp( self.complete_dynamic, (0,STEP*num), rv_0, method="RK45", rtol=1e-13, t_eval=range(0,STEP*num, STEP) ).y
		return ode_y
