...
for i in range(CSD_LM+3):
	...
	# PI_ij.append(1); 	# 补充1, 用于完全规格化和非规格化之间的转换
	PI_i0.extend(PI_ij); PI.append(np.array(PI_i0))
B, PI = np.array(B), np.array(PI)	# B用于完全规格化勒让德函数计算，PI用于摄动加速度计算
pi_dot = np.array([ np.array( [sqrt((i+1)*i/2)] + [ sqrt( (i+j+1)*(i-j) ) for j in range(1, i+1)] ) for i in range(CSD_LM+3) ])
...
CS2I = np.dot( Rx_bas(radians(24.35889)), Rz_bas(radians(-3.14227)) )	# 月心天球 -> 月心惯性系
...
class Orbit:
	...
	def moon_Cbi(self, tdb_jd):
		...
		return np.dot(CS2F, CS2I.T)	# 月惯系 -> 月固系, F = CS2F * I2CS * I
	...
	def nonspherGravity(self, r_sat, tdb_jd, miu=MIU_M, Re=RM, lm=CSD_LM):
		...
		lamda = atan2(r_fixed[1], r_fixed[0])
	...
	def nonspher_moon(self, r_sat, tdb_jd, miu=MIU_M, Re=RM, lm=CSD_LM):
		...
		a_fixed = np.dot( np.array([dU_dr, dU_dphi, dU_dlamda]),  spher2rect )
	...
	def nonspher_moongravity(self, r_sat, tdb_jd, miu=MIU_M, Re=RM, lm=CSD_LM):
		...
		r_norm = np.linalg.norm(r_fixed, 2)
		xy_norm = 1/np.linalg.norm(r_fixed[:2], 2)
		phic, lamda = asin(r_fixed[2]/r_norm), lamda = atan2(r_fixed[1], r_fixed[0])
		...
		for i in range(2, lm+1):
			...
			for j in range(i+1):
				dU_dr +=  P[i][j] * ( C[i][j] * cos(j*lamda) + S[i][j] * sin(j*lamda) ) * rRatio_n * (i+1)
				dU_dphi +=  ( P[i][j+1] * pi_dot[i][j] - j * r_fixed[2]*xy_norm * P[i][j] ) * \
							( C[i][j]*cos(j*lamda) + S[i][j]*sin(j*lamda) )* rRatio_n
				dU_dlamda += j * P[i][j] * ( S[i][j]*cos(j*lamda) - C[i][j]*sin(j*lamda) )* rRatio_n
	...
	def nonspher_matlab(self, r_sat, tdb_jd, miu=MIU_M, Re=RM, lm=CSD_LM):
		...
		a_inertial = a_fixed - (self.centreGravity(r_fixed/1000)) * 1000  # np.dot(I2F.T, a_fixed) / 1000		# m/s^2
		print(a_fixed, "\t\t", a_inertial)
		return a_inertial	# m/s^2
	...
	def thirdEarth(self, r_sat, tdb_jd):
		...
		miu = MIU_E
		r_earth = self.moon2Earth(tdb_jd)
	...
	def twobody_dynamic(self, t, RV,  miu=MIU_M):
		...
		drdv = []
		drdv.extend(RV[3:])
		drdv.extend(-miu*(RV[:3]) / pow(R_Norm, 3))
